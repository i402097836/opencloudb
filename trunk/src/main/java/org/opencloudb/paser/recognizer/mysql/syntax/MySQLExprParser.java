...
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.EOF;
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.IDENTIFIER;
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.KW_AND;
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.KW_AS;
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.KW_ASC;
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.KW_BY;
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.KW_COLLATE;
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.KW_DESC;
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.KW_DISTINCT;
...
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.KW_INTEGER;
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.KW_LIKE;
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.KW_NOT;
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.KW_SELECT;
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.KW_SEPARATOR;
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.KW_THEN;
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.KW_USING;
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.KW_WHEN;
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.KW_WITH;
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.LITERAL_CHARS;
...
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.PUNC_DOT;
...
import static org.opencloudb.paser.recognizer.mysql.MySQLToken.USR_VAR;
...
import org.opencloudb.paser.ast.expression.arithmeic.ArithmeticAddExpression;
import org.opencloudb.paser.ast.expression.arithmeic.ArithmeticDivideExpression;
import org.opencloudb.paser.ast.expression.arithmeic.ArithmeticIntegerDivideExpression;
import org.opencloudb.paser.ast.expression.arithmeic.ArithmeticModExpression;
import org.opencloudb.paser.ast.expression.arithmeic.ArithmeticMultiplyExpression;
import org.opencloudb.paser.ast.expression.arithmeic.ArithmeticSubtractExpression;
import org.opencloudb.paser.ast.expression.arithmeic.MinusExpression;
import org.opencloudb.paser.ast.expression.bit.BitAndExpression;
import org.opencloudb.paser.ast.expression.bit.BitInvertExpression;
import org.opencloudb.paser.ast.expression.bit.BitOrExpression;
import org.opencloudb.paser.ast.expression.bit.BitShiftExpression;
import org.opencloudb.paser.ast.expression.bit.BitXORExpression;
...
import org.opencloudb.paser.ast.expression.comparison.ComparisionGreaterThanExpression;
import org.opencloudb.paser.ast.expression.comparison.ComparisionGreaterThanOrEqualsExpression;
import org.opencloudb.paser.ast.expression.comparison.ComparisionIsExpression;
import org.opencloudb.paser.ast.expression.comparison.ComparisionLessOrGreaterThanExpression;
import org.opencloudb.paser.ast.expression.comparison.ComparisionLessThanExpression;
import org.opencloudb.paser.ast.expression.comparison.ComparisionLessThanOrEqualsExpression;
import org.opencloudb.paser.ast.expression.comparison.ComparisionNotEqualsExpression;
...
import org.opencloudb.paser.ast.expression.function.string.RegexpExpression;
import org.opencloudb.paser.ast.expression.function.string.SoundsLikeExpression;
...
import org.opencloudb.paser.ast.expression.function.string.Trim.Direction;
import org.opencloudb.paser.ast.expression.function.type.CastBinaryExpression;
...
import org.opencloudb.paser.ast.expression.logical.LogicalNotExpression;
import org.opencloudb.paser.ast.expression.logical.LogicalOrExpression;
import org.opencloudb.paser.ast.expression.logical.LogicalXORExpression;
import org.opencloudb.paser.ast.expression.logical.NegativeValueExpression;
import org.opencloudb.paser.ast.expression.misc.AssignmentExpression;
...
import org.opencloudb.paser.ast.expression.misc.SubqueryAllExpression;
import org.opencloudb.paser.ast.expression.misc.SubqueryAnyExpression;
...
import org.opencloudb.paser.ast.expression.primary.MatchExpression.Modifier;
            ...
            case KW_IS:
                ...
                case KW_NOT:
                    ...
                    case LITERAL_NULL:
                        lexer.nextToken();
                        fst =
                                new ComparisionIsExpression(fst, ComparisionIsExpression.IS_NOT_NULL).setCacheEvalRst(cacheEvalRst);
                        continue;
                    case LITERAL_BOOL_FALSE:
                        lexer.nextToken();
                        fst =
                                new ComparisionIsExpression(fst, ComparisionIsExpression.IS_NOT_FALSE).setCacheEvalRst(cacheEvalRst);
                        continue;
                    case LITERAL_BOOL_TRUE:
                        lexer.nextToken();
                        fst =
                                new ComparisionIsExpression(fst, ComparisionIsExpression.IS_NOT_TRUE).setCacheEvalRst(cacheEvalRst);
                        continue;
                    default:
                        matchIdentifier("UNKNOWN");
                        fst =
                                new ComparisionIsExpression(fst, ComparisionIsExpression.IS_NOT_UNKNOWN).setCacheEvalRst(cacheEvalRst);
                        continue;
                    }
                case LITERAL_NULL:
                    lexer.nextToken();
                    fst =
                            new ComparisionIsExpression(fst, ComparisionIsExpression.IS_NULL).setCacheEvalRst(cacheEvalRst);
                    continue;
                case LITERAL_BOOL_FALSE:
                    lexer.nextToken();
                    fst =
                            new ComparisionIsExpression(fst, ComparisionIsExpression.IS_FALSE).setCacheEvalRst(cacheEvalRst);
                    continue;
                case LITERAL_BOOL_TRUE:
                    lexer.nextToken();
                    fst =
                            new ComparisionIsExpression(fst, ComparisionIsExpression.IS_TRUE).setCacheEvalRst(cacheEvalRst);
                    continue;
                default:
                    matchIdentifier("UNKNOWN");
                    fst =
                            new ComparisionIsExpression(fst, ComparisionIsExpression.IS_UNKNOWN).setCacheEvalRst(cacheEvalRst);
                    continue;
                }
            case OP_EQUALS:
                lexer.nextToken();
    ...
    private Expression userExpression(String consumed, String consumedUp) throws SQLSyntaxErrorException {
        ...
        if (lexer.token() == USR_VAR) {
            if (first instanceof LiteralString) {
                ...
                StringBuilder str =
                        new StringBuilder().append(((Identifier) first).getIdText()).append(lexer.stringValue());
                lexer.nextToken();
                return new UserExpression(str.toString()).setCacheEvalRst(cacheEvalRst);
            }
        }
    ...
    private Expression primaryExpression(final String consumed, String consumedUp) throws SQLSyntaxErrorException {
        ...
        case LITERAL_HEX:
            LiteralHexadecimal hex =
                    new LiteralHexadecimal(null, lexer.getSQL(), lexer.getOffsetCache(), lexer.getSizeCache(), charset);
            lexer.nextToken();
    ...
    private Expression startedFromIdentifier(final String consumed, String consumedUp) throws SQLSyntaxErrorException {
        ...
        case PUNC_DOT:
            for (tempExpr = new Identifier(null, consumed, consumedUp).setCacheEvalRst(cacheEvalRst); lexer.token() == PUNC_DOT;) {
                ...
                case IDENTIFIER:
                    tempExpr =
                            new Identifier((Identifier) tempExpr, lexer.stringValue(), lexer.stringValueUppercase()).setCacheEvalRst(cacheEvalRst);
                    lexer.nextToken();
                    break;
                case OP_ASTERISK:
                    lexer.nextToken();
