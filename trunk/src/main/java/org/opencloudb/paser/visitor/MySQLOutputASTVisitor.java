...
import static org.opencloudb.paser.ast.expression.comparison.ComparisionIsExpression.IS_FALSE;
import static org.opencloudb.paser.ast.expression.comparison.ComparisionIsExpression.IS_NOT_FALSE;
import static org.opencloudb.paser.ast.expression.comparison.ComparisionIsExpression.IS_NOT_NULL;
import static org.opencloudb.paser.ast.expression.comparison.ComparisionIsExpression.IS_NOT_TRUE;
import static org.opencloudb.paser.ast.expression.comparison.ComparisionIsExpression.IS_NOT_UNKNOWN;
import static org.opencloudb.paser.ast.expression.comparison.ComparisionIsExpression.IS_NULL;
import static org.opencloudb.paser.ast.expression.comparison.ComparisionIsExpression.IS_TRUE;
import static org.opencloudb.paser.ast.expression.comparison.ComparisionIsExpression.IS_UNKNOWN;
...
import org.opencloudb.paser.ast.expression.primary.VariableExpression;
...
public final class MySQLOutputASTVisitor implements SQLASTVisitor {
    ...
    public Object[] getArguments() {
        ...
        if (argsIndexSize <= 0) return EMPTY_OBJ_ARRAY;
    ...
    public void visit(BetweenAndExpression node) {
        ...
        if (node.isNot()) appendable.append(" NOT BETWEEN ");
        else appendable.append(" BETWEEN ");
    ...
    public void visit(LikeExpression node) {
        ...
        if (node.isNot()) appendable.append(" NOT LIKE ");
        else appendable.append(" LIKE ");
    ...
    public void visit(PolyadicOperatorExpression node) {
        for (int i = 0, len = node.getArity(); i < len; ++i) {
            if (i > 0) appendable.append(' ').append(node.getOperator()).append(' ');
    ...
    public void visit(Trim node) {
        ...
        case BOTH:
            ...
            if (remStr != null) remStr.accept(this);
            appendable.append(" FROM ");
            break;
        case LEADING:
    ...
    public void visit(GroupConcat node) {
        ...
        if (orderBy != null) {
            ...
            orderBy.accept(this);
            if (node.isDesc()) appendable.append(" DESC");
            else appendable.append(" ASC");
    ...
    public void visit(LiteralBitField node) {
        ...
        if (introducer != null) appendable.append(introducer).append(' ');
    ...
    private void appendArgsIndex(int value) {
        ...
        if (argsIndex.length <= i) {
            int[] a = new int[i + 1];
            if (i > 0) System.arraycopy(argsIndex, 0, a, 0, i);
            argsIndex = a;
        }
        argsIndex[i] = value;
    }
    ...
    public void visit(OuterJoin node) {
        ...
        if (node.isLeftJoin()) appendable.append(" LEFT JOIN ");
        else appendable.append(" RIGHT JOIN ");
    ...
    public void visit(ShowBinLogEvent node) {
        ...
        if (logName != null) appendable.append(" IN ").append(logName);
    ...
    public void visit(ShowColumns node) {
        ...
        if (node.isFull()) appendable.append("FULL ");
