...
import org.opencloudb.paser.recognizer.mysql.syntax.MySQLDMLParser;
...
import org.opencloudb.paser.util.ListUtil;
...
public class MySQLDMLParserTest extends AbstractSyntaxTest {
    ...
    public void testTR1() throws SQLSyntaxErrorException {
        ...
        Assert.assertEquals("((SELECT * FROM any) UNION (SELECT `select` FROM `from` ORDER BY dd)) AS 'a1', t2, t3",
                output);
        ...
        Assert.assertEquals("(tb1 RIGHT JOIN tb2 USING ()) " + "INNER JOIN tb3 ON tb1.id = tb2.id AND tb2.id = tb3.id",
                output);
